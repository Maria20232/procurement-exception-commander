...
This fork turns the official template into a live procurement operations
interface backed by Supabase and Supervity Auto.

1. Copy `.env.example` to `.env`.
2. Set `SUPABASE_URL` and `SUPABASE_ANON_KEY`.
3. Set `SUPERVITY_WORKFLOW_URL` and, if required, `SUPERVITY_API_KEY`.
4. Run `docker compose up --build`.
5. Open `http://localhost:3001`.

Demo path: Dashboard → run `DN-5000` → Workbench → approve → AI Policies →
change a threshold and evaluate → Data Manager → show integration health.
